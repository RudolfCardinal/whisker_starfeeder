...
def to_bytes(s):
    if isinstance(s, six.binary_type):  # str in Python 2 and bytes in Python 3
        return s
    elif isinstance(s, six.text_type):  # unicode in Python 2 and str in Python 3
...
def to_string(data):
    # We want to return a str in both Python 2 and Python 3.
...
class Whisker(object):
    ...
    def send_immediate(self, s):
        ...
        self.immsock.sendall(to_bytes(s + "\n"))
...
class WhiskerTask(object):
    ...
    def send(self, msg):
        if not self.mainsocket:
            ...
            return
        self.mainsocket.send(to_bytes(msg))
...
class WhiskerMainPortProtocol(LineReceiver):
    ...
    def lineReceived(self, data):
        data = to_string(data)
        logger.debug("Main port received: {}".format(data))
        self.task.incoming_message(data)

    def send(self, data):
        logger.debug("Main port sending: {}".format(data))
...
class WhiskerImmSocket(object):
    ...
    def connect(self, server, port):
        logger.debug("WhiskerImmSocket: connect")
        proto = socket.getprotobyname("tcp")
        try:
            self.immsock = socket.socket(socket.AF_INET, socket.SOCK_STREAM,
                                         proto)
    ...
    def getlines_immsock(self):
        """Yield a set of lines from the socket."""
        # http://stackoverflow.com/questions/822001/python-sockets-buffering
        logger.debug("WhiskerImmSocket: getlines_immsock")
        buf = to_string(self.immsock.recv(WhiskerImmSocket.BUFFERSIZE))
        done = False
        while not done:
            if "\n" in buf:
                (line, buf) = buf.split("\n", 1)
    ...
    def send_and_get_reply(self, msg):
        ...
        self.immsock.sendall(to_bytes(msg + "\n"))
        reply = next(self.getlines_immsock())
        logger.debug("Immediate socket reply: " + reply)
        return reply
